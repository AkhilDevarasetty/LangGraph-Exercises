"""
Exercise 10 - Task 5: Production Features

Goal: Add rate limiting, timeouts, error handling, retry logic, and metrics

PRODUCTION FEATURES:
1. Rate Limiting - Prevent API overload (max 10 requests/minute)
2. Timeout Handling - Cancel streams after 30s
3. Error Recovery - Graceful degradation on failures
4. Retry Logic - Automatic retry with exponential backoff (max 3 attempts)
5. Metrics Dashboard - Track TTFT, latency, throughput, error rate

Based on: Task 4 (Multi-Mode Streaming)
"""
...
def main():
    ...
    while True:
        ...
        # Manual timeout tracking (works with sync .stream())
        TIMEOUT_SECONDS = 30
        timed_out = False
        ...
        try:
            ...
            if timed_out:
                print(
                    f"\n\n{Fore.RED}⏰ Response timed out after {TIMEOUT_SECONDS}s. Please try a simpler question.{Style.RESET_ALL}"
                )
            else:
                # Calculate and print metrics (only if not timed out)
