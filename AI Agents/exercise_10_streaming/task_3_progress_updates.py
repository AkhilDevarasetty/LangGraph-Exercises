"""
Exercise 10 - Task 3: Custom Progress Updates

Goal: Show real-time status updates during agent execution

YOUR TASK:
1. Import get_stream_writer from langgraph.config
2. Add progress updates to routing node (before/after routing)
3. Add progress updates to retrieval node (before/after retrieval)
4. Use stream_mode="custom" to receive progress updates
5. Display progress updates with timestamps
6. Combine with token streaming for complete UX

Based on: Task 2 (Async Retrieval)
"""
...
def main():
    ...
    print("\n" + "=" * 80)
    print("✨ MULTI-DOC RAG AGENT WITH PROGRESS UPDATES!")
    print("=" * 80)
    ...
    while True:
        ...
        # TODO 5: Use multi-mode streaming to get both messages AND custom progress
        # Hint: Change stream_mode to a list: stream_mode=["messages", "custom"]
        # This returns tuples: (mode, chunk) instead of (chunk, metadata)
        ...
        # TODO 6: Update loop to handle multiple modes
        # Hint: for mode, chunk in llm_response:
        #           if mode == "messages":
        #               # Handle token streaming (like before)
        #           elif mode == "custom":
        ...
        for mode, chunk in llm_response:
            if mode == "messages":
                ...
                print(f"\n[{elapsed:.2f}s] {status}", flush=True)
